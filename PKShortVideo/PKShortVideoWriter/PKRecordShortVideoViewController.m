...
- (void)viewDidLoad {
    ...
    //圆圈计时器
    tbcycleView=[[TBCycleView alloc]init];
    tbcycleView.frame = CGRectMake(0, 0, PKRecordButtonWidth+15, PKRecordButtonWidth+15);
    tbcycleView.center = CGPointMake(kScreenWidth/2, PKRecordButtonVarticalHeight);
    tbcycleView.backgroundColor=[UIColor clearColor];
    [self.view addSubview:tbcycleView];
    num=0.1;
    ...
    [transformBtn addTarget:self action:@selector(swapCamera) forControlEvents:UIControlEventTouchUpInside];
    [transformBtn setImage:[UIImage imageNamed:@"PK_Camera_Turn"] forState:UIControlStateNormal];
    transformBtn.frame=CGRectMake(kScreenWidth-44, 0, 40, 40);
    transformBtn.tintColor = [UIColor whiteColor];
    [self.view addSubview:transformBtn];

    //返回按钮
    UIButton *backBtn = [[UIButton alloc] init];
    [backBtn addTarget:self action:@selector(backClick) forControlEvents:UIControlEventTouchUpInside];
    backBtn.frame=CGRectMake(44, 0, 40, 40);
...
-(void)loop{
    ...
    [tbcycleView drawProgress:num/80.0];
}
...
- (void)sendVideo {
    UIImage *image = [UIImage pk_previewImageWithVideoURL:[NSURL fileURLWithPath:self.outputFilePath]];
    PKScreenViewController *vc = [[PKScreenViewController alloc] initWithVideoPath:self.outputFilePath previewImage:image];
    NSData * imageData = UIImageJPEGRepresentation(image,1);
    
    NSLog(@"大小%lu",[imageData length]/1000);
    [self presentViewController:vc animated:NO completion:NULL];
}
...
- (void)invalidateTime {
    // (CACurrentMediaTime() - self.beginTime)/self.duration
...
- (void)recorderDidEndRecording:(PKShortVideoRecorder *)recorder {
    //停止进度条
    NSLog(@"nnnnn");
    NSLog(@"begin %f     %f",self.beginRecordTime,CACurrentMediaTime());
