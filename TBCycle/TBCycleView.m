...
- (void)drawRect:(CGRect)rect {
    [self drawCycleProgressWithX:self.center.x-self.frame.origin.x Y:self.center.y-self.frame.origin.y Z:self.frame.size.width/2-5];
    NSLog(@"%f  %f",self.center.x-100,self.center.x);
}

- (void)drawCycleProgressWithX:(float)xz Y:(float)yz Z:(float)zz
{
    CGPoint center = CGPointMake(xz, yz);
    CGFloat radius = zz;
    ...
    rightLayer.colors = @[(id)[UIColor greenColor].CGColor, (id)[UIColor greenColor].CGColor];
